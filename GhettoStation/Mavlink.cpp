...
void request_mavlink_rates()
{
	...
	{ MAV_DATA_STREAM_RAW_SENSORS, MAV_DATA_STREAM_EXTENDED_STATUS,
	...
	{
		mavlink_msg_request_data_stream_pack(127, 0, &msg, 7, 1, MAVStreams[i],
				MAVRates[i], 1);
	...
	while (TELEMETRY_SERIAL.available() > 0)
	{
		...
		if (mavlink_parse_char(MAVLINK_COMM_0, c, &msg, &status))
		{
			...
			case MAVLINK_MSG_ID_HEARTBEAT:
			{
				...
				uav_flightmode =
						(uint8_t) mavlink_msg_heartbeat_get_custom_mode(&msg);
				if (apm_mav_type == 2) //ArduCopter MultiRotor or ArduCopter Heli
				{
					...
					case 11:
						uav_flightmode = 5;
						break;      //Drift -> Stabilize1
					case 12:
						uav_flightmode = 6;
						break;      //Sport -> Stabilize2
					default:
						uav_flightmode = 19;
						break;      //Unknown
					}
			...
			case MAVLINK_MSG_ID_SYS_STATUS:
			{
				...
				uav_current = mavlink_msg_sys_status_get_current_battery(&msg)
						/ 10;
				uav_amp =
						(int16_t) mavlink_msg_sys_status_get_battery_remaining(
								&msg); //Mavlink send battery remaining % , will use this instead
			}
				break;
...
#endif
				uav_fix_type = (uint8_t) mavlink_msg_gps_raw_int_get_fix_type(
						&msg);
				uav_satellites_visible =
						(uint8_t) mavlink_msg_gps_raw_int_get_satellites_visible(
								&msg);
				uav_gpsheading = (int16_t) mavlink_msg_gps_raw_int_get_cog(
						&msg);
			}
				break;
			case MAVLINK_MSG_ID_VFR_HUD:
			{
				uav_groundspeed = (uint16_t) round(
						mavlink_msg_vfr_hud_get_groundspeed(&msg));
				uav_airspeed = (uint8_t) round(
						mavlink_msg_vfr_hud_get_airspeed(&msg));
#ifdef BARO_ALT
				uav_alt = (int32_t) round(
						mavlink_msg_vfr_hud_get_alt(&msg) * 100.0f); // from m to cm
#endif
			}
				break;
			case MAVLINK_MSG_ID_ATTITUDE:
			{
				uav_roll = (int16_t) round(
						toDeg(mavlink_msg_attitude_get_roll(&msg)));
				uav_pitch = (int16_t) round(
						toDeg(mavlink_msg_attitude_get_pitch(&msg)));
				uav_heading = (int16_t) round(
						toDeg(mavlink_msg_attitude_get_yaw(&msg)));
