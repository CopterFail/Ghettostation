...
void ltm_read()
{
	...
	{
		IDLE, HEADER_START1, HEADER_START2, HEADER_MSGTYPE, HEADER_DATA
	} c_state = IDLE;
...
void ltm_check()
{
	...
	if (LTMcmd == LIGHTTELEMETRY_AFRAME)
	{
		...
		if (uav_heading < 0)
			uav_heading = uav_heading + 360; //convert from -180/180 to 0/360°
...
void send_LTM_Gframe()
{
	...
	LTBuff[16] = ((uav_satellites_visible << 2) & 0xFF)
			| (uav_fix_type & 0b00000011); // last 6 bits: sats number, first 2:fix type (0,1,2,3)
...
static void send_LTM_Sframe()
{
	...
	LTBuff[9] = ((uav_flightmode << 2) & 0xFF)
			| ((uav_failsafe << 1) & 0b00000010) | (uav_arm & 0b00000001);
...
static void send_LTM_Oframe()  // this frame is only dedicated to OSD.
{
