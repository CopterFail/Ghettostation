...
void setup()
{
	...
	// attach servos
	attach_servo(pan_servo, PAN_SERVOPIN, configuration.pan_minpwm,
			configuration.pan_maxpwm);
	attach_servo(tilt_servo, TILT_SERVOPIN, configuration.tilt_minpwm,
			configuration.tilt_maxpwm);
...
void check_activity(void)
{
	...
	case ActPanMinPwm:             //PAN_MINPWM
		...
		if (servoconf_tmp[0] != servoconfprev_tmp[0])
		{
			...
			attach_servo(pan_servo, PAN_SERVOPIN, servoconf_tmp[0],
					configuration.pan_maxpwm);
		}
	...
	case ActPanMinAngle:             //PAN_MINANGLE
		configuration.pan_minangle = config_servo(1, 2,
	...
	case ActPanMaxAngle:             //PAN_MAXANGLE
		configuration.pan_maxangle = config_servo(1, 4,
	...
	case ActTiltMinPwm:             //"TILT_MINPWM"
		...
		if (servoconf_tmp[2] != servoconfprev_tmp[2])
		{
			...
			attach_servo(tilt_servo, TILT_SERVOPIN, servoconf_tmp[2],
					configuration.tilt_maxpwm);
		}
	...
	case ActTiltMinAngle:             //TILT_MINANGLE
		configuration.tilt_minangle = config_servo(2, 2,
	...
	case ActTiltMaxAngle:                //TILT_MAXANGLE
		configuration.tilt_maxangle = config_servo(2, 4,
	...
	if (buttonEnter.holdTime() < 700) // normal press
	{
		...
		else if (current_activity == ActSetHome)
		{
			...
			else if ((gps_fix) && (home_pos) && (!home_bear))
			{
				...
				{
				case 1:
					home_bearing += calc_bearing(home_lon, home_lat, uav_lon,
							uav_lat); // store bearing relative to north
					home_bear = true;
					break;
				case 2:
				case 3:
...
void leftButtonReleaseEvents(Button &btn)
{
	if (buttonDown.holdTime() < 700)
	{
		...
		else if (current_activity != ActMenu && current_activity != ActTrack
				&& current_activity != ActSetHome)
		{
			...
			case ActTiltMinPwm:
				servoconf_tmp[2]--;
				break;
			case ActTiltMinAngle:
				configuration.tilt_minangle--;
				break;
			case ActTiltMaxPwm:
				servoconf_tmp[3]--;
				break;
			case ActTiltMaxAngle:
				configuration.tilt_maxangle--;
				break;
			case ActSetTelemetrie:
			...
			case ActSetVoltage:
				...
				break;
			case ActSetChannel:
				configuration.channel = (configuration.channel - 1) & 0x1f;
				break;
			}
		}
		else if (current_activity == ActSetHome)
...
//find the best way to move pan servo considering 0° reference face toward
	...
	if (inverse)
		i16a = 180 - i16a;
	if (i16a < configuration.tilt_minangle)
		i16a = configuration.tilt_minangle;
	if (i16a > configuration.tilt_maxangle)
		i16a = configuration.tilt_maxangle;

	// write the servos:
	//microsec = map(a, 0, i16PanRange, configuration.pan_minpwm, configuration.pan_maxpwm);
	microsec = map(i16b, 0, i16PanRange, configuration.pan_maxpwm,
			configuration.pan_minpwm);	// pan servo move counter clockwise....
	pan_servo.writeMicroseconds(microsec);

	microsec = map(i16a, configuration.tilt_minangle,
			configuration.tilt_maxangle, configuration.tilt_minpwm,
			configuration.tilt_maxpwm);		// tilt servo
...
void calc_tracking(int32_t lon1, int32_t lat1, int32_t lon2, int32_t lat2,
		int32_t alt)
{
...
#define BATT_COUNT 9
	...
	{
		while (voltage_actual > ((s + 1) * LIPO_MIN_VOLTAGE))
			s++;
	}
