...
void uavtalk_send_msg(uavtalk_message_t *msg)
{
	...
	TELEMETRY_SERIAL.write(msg->Crc);
}
...
void uavtalk_send_gcstelemetrystats(void)
{
	...
	if (gcstelemetrystats_obj_len == 0)
		return;	// we didn't know yet with which version we are talking, so be quiet
...
void set_telemetrystats_values(uint32_t ObjID)
{
	...
	if (gcstelemetrystats_obj_len != 0)
		return;	// we already know yet with which version we are talking, so do nothing
	...
	while (TELEMETRY_SERIAL.available() > 0)
	{
		...
		if (uavtalk_parse_char(c, &msg))
		{
			...
			case ATTITUDESTATE_OBJID:
				last_flighttelemetry_connect = millis();
				show_prio_info = 1;
				uav_roll = (int16_t) uavtalk_get_float(&msg,
						ATTITUDEACTUAL_OBJ_ROLL);
				uav_pitch = (int16_t) uavtalk_get_float(&msg,
						ATTITUDEACTUAL_OBJ_PITCH);
				uav_heading = (int16_t) uavtalk_get_float(&msg,
						ATTITUDEACTUAL_OBJ_YAW);
				break;
			...
			case FLIGHTSTATUS_OBJID_005:
				...
				case 4:
					uav_flightmode = 16;
					break;   //autotune (unknown)
				case 5:
					uav_flightmode = 8;
					break;   //altitude hold
				case 6:
					uav_flightmode = 16;
					break;   //velocity control (unknown)
				case 7:
					uav_flightmode = 9;
					break;   //pos hold
				case 8:
				...
				case 9:
					uav_flightmode = 10;
					break;   //pathplanner (auto)
				}

				//if (msg.ObjID==FLIGHTSTATUS_OBJID_004) {
				uav_failsafe =
						(uavtalk_get_int8(&msg, FLIGHTSTATUS_OBJ_CONTROLSOURCE)
								== 1) ? 1 : 0; // Taulabs only
				//}
				break;
...
//					uav_chan8_raw		= uavtalk_get_int16(&msg, MANUALCONTROLCOMMAND_OBJ_CHANNEL_8);
			...
			case MANUALCONTROLCOMMAND_OBJID_001: //Taulabs
				uav_rssi = (uint8_t) uavtalk_get_int16(&msg,
						MANUALCONTROLCOMMAND_OBJ_001_RSSI);
				if ((uav_rssi < 0) || uav_rssi > 100)
					uav_rssi = 0; // rssi > 100 means it entered taulabs failsafe mode
...
//					uav_chan8_raw		= uavtalk_get_int16(&msg, MANUALCONTROLCOMMAND_OBJ_001_CHANNEL_8);
			...
			case GPSPOSITIONSENSOR_OBJID:
				...
				uav_satellites_visible = (uint8_t) uavtalk_get_int8(&msg,
						GPSPOSITION_OBJ_SATELLITES);
				uav_fix_type = (uint8_t) uavtalk_get_int8(&msg,
						GPSPOSITION_OBJ_STATUS);
				uav_gpsheading = (int16_t) uavtalk_get_float(&msg,
						GPSPOSITION_OBJ_HEADING);
#ifndef BARO_ALT
				uav_alt = (int32_t) round (uavtalk_get_float(&msg, GPSPOSITION_OBJ_ALTITUDE) * 100.0f);
#endif
				uav_groundspeed = (uint16_t) uavtalk_get_float(&msg,
						GPSPOSITION_OBJ_GROUNDSPEED);
				break;

			case FLIGHTBATTERYSTATE_OBJID:
				uav_bat = (int16_t)(
						1000.0
								* uavtalk_get_float(&msg,
										FLIGHTBATTERYSTATE_OBJ_VOLTAGE));
				uav_current = (int16_t)(
						100.0
								* uavtalk_get_float(&msg,
										FLIGHTBATTERYSTATE_OBJ_CURRENT));
				uav_amp = (int16_t) uavtalk_get_float(&msg,
						FLIGHTBATTERYSTATE_OBJ_CONSUMED_ENERGY);
				break;

			case BAROALTITUDE_OBJID:
			case BAROSENSOR_OBJID:
#ifdef BARO_ALT
				uav_alt = (int32_t) round(
						uavtalk_get_float(&msg, BAROALTITUDE_OBJ_ALTITUDE)
								* 100.0f);
#endif
				break;
			...
			case OPLINKSTATUS_OBJID_001:
				...
				uav_linkquality = uavtalk_get_int8(&msg,
						OPLINKSTATUS_OBJ_LINKQUALITY);
				break;
	...
	if (last_flighttelemetry_connect + FLIGHTTELEMETRYSTATS_CONNECT_TIMEOUT
			< millis())
	{
